...
- [ ] **Step 5: 对真实库做一次只读 sanity check**

Run: `python3 stages/scoring.py`
Expected: 正常输出"待评分机会点：N 个 / 评分完成"，无异常；随后 `sqlite3 data/pipeline.db "SELECT status, COUNT(*) FROM opportunities GROUP BY status;"` 确认没有行的 status 被意外改动（open 数不因本次运行减少——本次运行只应给 value IS NULL 的行打分）
...
Expected: 输出"待核账机会点：N 个"、逐条 dry-run 动作、僵尸巡检结果；DB 无任何写入（`git diff data/pipeline.db` 为空）
...
Expected: 输出"baseline 已快照：10/10 条 golden 命中"（若有 MISSING 说明 source_ref 抄写有误，回 Step 5 修正）
...
- **corrected**（机会点成立但证据有误）：先修正对应 evidence JSON，再：
  ...
  WHERE id=<id>;
  ```
  （四评分列置 NULL 是触发 scoring.py 复评的唯一机制，必须全部置 NULL；evidence 用参数化查询写入，先 `json.dumps()`。）
...
- [ ] **Step 2: 复制 run_v3_verify 函数**

把 Task 9 Step 2 的 `run_v3_verify()` 函数完整复制到 `run_bulk_v2.sh` 的锁检查块之后、主流程之前（该脚本已有 `$DB`/`$STAGES`/`$PROMPTS`/`$PIPELINE_DIR`/`$CLI_TOOL` 同名变量，函数体零修改可用）。
